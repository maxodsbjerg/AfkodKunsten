...
---
The focal point of this little example is to proces an extract of the collection data from the National Gallery of Denmark. This collection is comprised of the museum’s digitally registered art works.
...
Since the data now has a format where each instance of an art work on display has its own row, we can now start asking questions about the exhibitioning of the our period.
...
```{r}
golden_paintings_exh %>% 
  count(title, creator_1, sort = TRUE)
```
